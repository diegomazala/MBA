...
static void create_grid_mesh(TriMesh& mesh, uint32_t rows, uint32_t cols, const OpenMesh::Vec3f& up = {0, 1, 0},float width = 1.0f, float height = 1.0f)
{
    ...
    for (auto i = 0; i <= rows; ++i)
    {
        for (auto j = 0; j <= cols; ++j)
        {
            ...
            //vhandle[i*(rows+1)+j] = mesh.add_vertex(TriMesh::Point(x, z * up[2], z * up[1]));
...
static void mesh_to_vecs(TriMesh& mesh, 
    std::shared_ptr<std::vector<decimal>> U, 
