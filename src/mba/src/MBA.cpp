...
//===========================================================================
const std::vector<UCBspl_real> MBA::smoothing_filter_ =
    generate_smoothing_filter();
//===========================================================================
...
bool MBA::adjustForBaseSurface()
{
    ...
    if (data_.baseType_ != MBA_ZERO)
    {
        ...
        (*PHI_) +=
            data_
                .offset_; // add to every coefficient in the matrix (by an operator)
        data_.offset_ = 0.0;
...
//===========================================================================
{
    ...
    for (int ip = 0; ip < noPoints; ip++)
    {
        ...
        int offset =
            1; // for 4x4 neighbourhood (does not affect approximation accuracy)
...
{ // original domain
    ...
    // NOTE for maintanance : This is the same algorithm as
    // UCBspl::SplineSurface::f(u,v) apart from
    // adding base surface
...
void MBA::checkSparsity() const
{
    ...
    cout << "Entries in PHI_(" << m_ << "," << n_ << "): " << (m_ + 3) * (n_ + 3)
         << endl;
    cout << "No. of zeros: " << no_zeros << endl;
    cout << "Sparsity: "
         << (double)no_zeros / (double)((m_ + 3) * (n_ + 3)) * 100.0 << endl;
}
...
void MBA::BAalg()
{
    ...
    std::cout 
        << "Data domain: " 
        << data_.umin() << ' '<< data_.vmin() << ' '
        << data_.umax() << ' '<< data_.vmax() << ' '
        << data_.rangeUInv() << ' '<< data_.rangeVInv() << '\n';

    double min_uc = std::numeric_limits<double>::max();;
    double min_vc = std::numeric_limits<double>::max();
    double max_uc = 0;
    double max_vc = 0;
    for (int ip = 0; ip < noPoints; ip++)
    {
        ...
        for (k = 0; k <= 3; k++)
        {
            for (l = 0; l <= 3; l++)
            {
                ...
                // double alpha = 0.01; double phi_kl = (tmp*zc + alpha*(tmp -
                // sum_w_ab2)) / sum_w_ab2;
...
void MBA::MBAalg(int m0, int n0, int h, int num_smoothing)
{
    ...
    while (k <= h)
    {
        ...
        if (k != 0)
        {
            PSI += PSIprime; // PSI = PHI_: exchange arrays (simpler WHEN k = 0, then
                             // PSIprime = 0)
        }
...
#else
            UCBspl::refineCoeffsC2(
                PSI, PSIprime); // refining coefficients and storing in PSIprime
    ...
    //
    // PHI_ += data_.offset_; // or on all coefficients by data_.f(u,v) for
    // LS-base.
    // But, if the algorithm should be run further later? (translate the points
    // before starting)
    // data_.offset_ = 0.0;

    // free memory. Later, an interactive version with more refinement may use
    // them.
...
vector<UCBspl_real> generate_smoothing_filter()
{
    ...
    UCBspl_real data[] = {-1, 24, 14, 24, -1, 24, -56, -176, -56,
                          24, 14, -176, 0, -176, 14, 24, -56, -176,
                          -56, 24, -1, 24, 14, 24, -1};
...
UCBspl_real extrapolate_point(int i, int j, const GenMatrixType &matrix)
{
    ...
    if (i < -1)
    {
        if (j < -1)
        {
            int ix = -i;
            int jx = -j;
            return (ix + 1) * (jx + 1) * matrix(-1, -1) -
                   (ix + 1) * jx * matrix(0, -1) - ix * (jx + 1) * matrix(-1, 0) +
                   ix * jx * matrix(0, 0);
        }
        else if (j >= resY - 1)
        {
            int ix = -i;
            int jx = j - (resY - 2);
            return (ix + 1) * (jx + 1) * matrix(-1, resY - 2) -
    ...
    else if (i >= resX - 1)
    {
        if (j < -1)
        {
            int ix = i - (resX - 2);
            int jx = -j;
            return (ix + 1) * (jx + 1) * matrix(resX - 2, -1) -
        ...
        else if (j >= resY - 1)
        {
            int ix = i - (resX - 2);
            int jx = j - (resY - 2);
            return (ix + 1) * (jx + 1) * matrix(resX - 2, resY - 2) -
