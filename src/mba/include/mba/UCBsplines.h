...
// Basis functions
inline double B_0(double t)
{
    double tmp = 1.0 - t;
    return tmp * tmp * tmp / 2.0;
}
...
static double tensor_BB[2][2] = {{1. / 4., 1. / 4.}, {1. / 4., 1. / 4.}};

// ???? only temp. for compilation
// equivalent to dB(k,s)*B(l,t).
static double tensor_dBB[2][2] = {{-3. / 4., -3. / 4.}, {3. / 4., 3. / 4.}};

// The transpose of the matrix above, for the y-components,
// equivalent to B(k,s)*dB(l,t)
static double tensor_BdB[2][2] = {{-3. / 4., 3. / 4.}, {-3. / 4., 3. / 4.}};
...
// static inline double B_2(double t) {return (-3.0*t*t*t + 3.0*t*t + 3.0*t +
// 1.0)/6.0;}
static double div16 = 1.0 / 6.0;
inline double B_2(double t)
{
    return (-0.5 * t * t * t + 0.5 * t * t + 0.5 * t + div16);
}
...
// Pre-evaluated tensors equivalent to B(k,s)*B(l,t)
static double tensor_BB[3][3] = {{1. / 36., 1. / 9., 1. / 36.},
                                 {1. / 9., 4. / 9., 1. / 9.},
                                 {1. / 36., 1. / 9., 1. / 36.}};
...
static double tensor_dBB[3][3] = {{-1. / 12., -1. / 3., -1. / 12.},
                                  {0., 0., 0.},
                                  {1. / 12., 1. / 3., 1. / 12.}};

// The transpose of the matrix above, for the y-components,
// equivalent to B(k,s)*dB(l,t)
static double tensor_BdB[3][3] = {{-1. / 12., 0., 1. / 12.},
                                  {-1. / 3., 0., 1. / 3.},
                                  {-1. / 12., 0., 1. / 12.}};
...
inline double w(int k, int l, double s, double t) { return B(k, s) * B(l, t); }

// s,t \in [0,1) (but special on gridlines m and n)
// i,j \in [-1, ???
inline void ijst(int m, int n, double uc, double vc, int &i, int &j, double &s,
                 double &t)
{
...
// adjust for x or y on gridlines m and n (since impl. has 0 <= x <= m and 0 <=
// y <= n
#ifdef UNIFORM_CUBIC_C1_SPLINES
    if (i == 2 * m - 1)
    {
...
inline void WKLandSum2(double s, double t, double w_kl[4][4],
                       double &sum_w_ab2)
{
    sum_w_ab2 = 0.0;
    double Bs0 = B_0(s);
...
{
    return 1.0 / 64.0 * (mat(i - 1, j - 1) + mat(i - 1, j + 1) + mat(i + 1, j - 1) + mat(i + 1, j + 1) + 6.0 * (mat(i - 1, j) + mat(i, j - 1) + mat(i, j + 1) + mat(i + 1, j)) + 36.0 * mat(i, j));
}
...
{
    return 1.0 / 16.0 * (mat(i - 1, j) + mat(i - 1, j + 1) + mat(i + 1, j) + mat(i + 1, j + 1) + 6.0 * (mat(i, j) + mat(i, j + 1)));
}
...
{
    return 1.0 / 16.0 * (mat(i, j - 1) + mat(i, j + 1) + mat(i + 1, j - 1) + mat(i + 1, j + 1) + 6.0 * (mat(i, j) + mat(i + 1, j)));
}
...
{
    return 1.0 / 64.0 *
           (mat(i - 1, j - 1) + 5.0 * (mat(i - 1, j) + mat(i, j - 1)) +
            25.0 * mat(i, j) + 2.0 * (mat(i - 1, j + 1) + mat(i + 1, j - 1)) +
            10.0 * (mat(i + 1, j) + mat(i, j + 1)) + 4.0 * mat(i + 1, j + 1));
}
...
{
    return 1.0 / 64.0 *
           (4.0 * mat(i, j) + 5.0 * mat(i + 2, j + 1) +
            2 * (mat(i + 2, j) + mat(i, j + 2)) + mat(i + 2, j + 2) +
            25 * mat(i + 1, j + 1) + 10.0 * (mat(i + 1, j) + mat(i, j + 1)) +
            5.0 * mat(i + 1, j + 2));
}
...
{
    return 1.0 / 32.0 *
           (3.0 * mat(i - 1, j - 1) + 15.0 * mat(i - 1, j) + mat(i, j - 1) +
            5.0 * mat(i, j) + 2.0 * mat(i, j + 1) + 6.0 * mat(i - 1, j + 1));
}
...
{
    return 1.0 / 32.0 *
           (3.0 * mat(i - 1, j - 1) + mat(i - 1, j) + 15 * mat(i, j - 1) +
            5.0 * mat(i, j) + 2.0 * mat(i + 1, j) + 6.0 * mat(i + 1, j - 1));
}
...
{
    return 1.0 / 32.0 *
           (6.0 * mat(i - 1, j) + 2.0 * mat(i, j) + 15 * mat(i - 1, j + 1) +
            5.0 * mat(i, j + 1) + mat(i, j + 2) + 3.0 * mat(i - 1, j + 2));
}
...
{
    return 1.0 / 32.0 *
           (mat(i - 1, j - 1) + 5.0 * mat(i - 1, j) + 3.0 * mat(i, j - 1) +
            15.0 * mat(i, j) + 2.0 * mat(i - 1, j + 1) + 6.0 * mat(i, j + 1));
}
...
{
    return 1.0 / 32.0 *
           (mat(i - 1, j - 1) + 3.0 * mat(i - 1, j) + 5.0 * mat(i, j - 1) +
            15.0 * mat(i, j) + 2.0 * mat(i + 1, j - 1) + 6.0 * mat(i + 1, j));
}
...
{
    return 1.0 / 32.0 *
           (2.0 * mat(i - 1, j) + 6.0 * mat(i, j) + 5.0 * mat(i - 1, j + 1) +
            15.0 * mat(i, j + 1) + mat(i - 1, j + 2) + 3.0 * mat(i, j + 2));
}
...
{
    return 1.0 / 32.0 *
           (2.0 * mat(i, j - 1) + 6.0 * mat(i, j) + 5.0 * mat(i + 1, j - 1) +
            15.0 * mat(i + 1, j) + mat(i + 2, j - 1) + 3.0 * mat(i + 2, j));
}
...
{
    return 1.0 / 64.0 *
           (2.0 * (mat(i - 1, j) + mat(i + 1, j + 2)) +
            10.0 * (mat(i, j) + mat(i + 1, j + 1)) +
            5.0 * (mat(i - 1, j + 1) + mat(i, j + 2)) + 25.0 * mat(i, j + 1) +
            4.0 * mat(i + 1, j) + mat(i - 1, j + 2));
}
...
{
    return 1.0 / 64.0 *
           (2.0 * (mat(i, j - 1) + mat(i + 2, j + 1)) +
            10.0 * (mat(i, j) + mat(i + 1, j + 1)) +
            5.0 * (mat(i + 1, j - 1) + mat(i + 2, j)) + 4.0 * mat(i, j + 1) +
            25.0 * mat(i + 1, j) + mat(i + 2, j - 1));
}

// Refinement (Similar to the Oslo algorithm)
void refineCoeffsC1(const GenMatrix<UCBspl_real> &PSI,
                    GenMatrix<UCBspl_real> &PSIprime);
void refineCoeffsC2(const GenMatrix<UCBspl_real> &PSI,
                    GenMatrix<UCBspl_real> &PSIprime);

// Restriction (This is not used by Multilvel B-splines) "Transposed" of the
// refinement operator
bool restrictCoeffsC2(const GenMatrix<UCBspl_real> &rr,
                      GenMatrix<UCBspl_real> &r);
